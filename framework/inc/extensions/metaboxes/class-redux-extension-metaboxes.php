					...
					foreach ( $sections as $s_key => $section ) {
						...
						if ( isset( $section['fields'] ) && ! empty( $section['fields'] ) ) {
							...
							foreach ( $section['fields'] as $f_key => $field ) {
								...
								echo '<tr valign="top"' . esc_html($ex_style) . '>'; // phpcs:ignore WordPress.Security.EscapeOutput
								...
								if ( $sidebar ) {
									if ( ! ( isset( $metabox['args']['sections'] ) && 1 === count( $metabox['args']['sections'] ) && isset( $metabox['args']['sections'][0]['fields'] ) && 1 === count( $metabox['args']['sections'][0]['fields'] ) ) && isset( $field['title'] ) ) {
										echo '<th scope="row">';
										if ( ! empty( $th ) ) {
											echo esc_attr($th); // phpcs:ignore WordPress.Security.EscapeOutput
										}
										echo '</th>';
										echo '<td>';
									}
								} else {
									echo '<td>' . esc_attr($th) . ''; // phpcs:ignore WordPress.Security.EscapeOutput
								}
